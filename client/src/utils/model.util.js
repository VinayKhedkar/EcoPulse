...
async function runONNXModel(imageFile) {
  ...
  const arr = output.dense.cpuData;
...
const preprocessImage = async (file) => {
  return new Promise((resolve, reject) => {
    if (!(file instanceof Blob || file instanceof File)) {
      return reject(
        new Error("Invalid file object passed to preprocessImage.")
      );
    }
    ...
    img.onload = () => {
      ...
      for (let y = 0; y < 224; y++) {
        for (let x = 0; x < 224; x++) {
          ...
          floatArray[0 * 224 * 224 + y * 224 + x] = data[pixelIdx] / 255.0;
          floatArray[1 * 224 * 224 + y * 224 + x] = data[pixelIdx + 1] / 255.0;
          floatArray[2 * 224 * 224 + y * 224 + x] = data[pixelIdx + 2] / 255.0;
        }
      }
